...
import { User, Withdrawal, TransactionStatus, UserRole } from "../types.js";
...
export function WithdrawalsView({
  ...
  const [queuedEntries, setQueuedEntries] = useState<TempWithdrawalEntry[]>([]);
  ...
  const handleQueuePlus = (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      // Add new entry
      const newEntry: TempWithdrawalEntry = {
        tempId: `temp-${Date.now()}`,
        amount: Number(amount),
        date,
        chequeNumber,
        reference: reference || `WTH-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
        notes,
    ...
    // Reset Form Input except target shareholder
    setAmount("");
    setChequeNumber("");
    setReference("");
    setNotes("");
  ...
  const handleEditQueued = (entry: TempWithdrawalEntry) => {
  ...
  const handleSubmitBatch = async () => {
    ...
    try {
      ...
      setQueuedEntries([]);
      onRefreshData();
      alert("Successfully submitted withdrawal requests for board review.");
    } catch (e: any) {
      alert(e.message || "Failed to submit withdrawals batch.");
    } finally {
  ...
  const handleDecline = async () => {
    ...
    try {
      await api.declineWithdrawal(id, declineRemarks);
      onRefreshData();
    } catch (e: any) {
      alert(e.message || "Failed to decline withdrawal.");
    } finally {
      setProcessingId(null);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    setDeleteError(null);
    try {
      const res = await api.deleteWithdrawal(deleteConfirmId);
      if (res.success) {
        onRefreshData();
        setDeleteConfirmId(null);
      } else {
        setDeleteError(res.message || "Failed to delete withdrawal transaction.");
      }
    } catch (err: any) {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-5 bg-white rounded-2xl border border-slate-100 p-6 shadow-xs">
          <h3 className="text-base font-display font-semibold text-slate-800 mb-4">
            {editingId ? "Edit Queued Withdrawal Entry" : "Queue New Withdrawal Entry"}
          </h3>
          ...
          <form onSubmit={handleQueuePlus} className="space-y-4">
            ...
            <div>
              ...
              <textarea
                rows={2}
                placeholder="Explain withdrawal purpose..."
                value={notes}
            ...
            <button
              type="submit"
              className="w-full inline-flex items-center justify-center gap-2 py-2.5 px-4 border border-transparent rounded-xl shadow-xs text-xs font-bold text-white bg-rose-600 hover:bg-rose-700 focus:outline-hidden transition-all cursor-pointer"
            >
              <Plus className="w-4 h-4" />
              {editingId ? "Save Withdrawal Details" : "Add to Batch List"}
            </button>
          </form>
        </div>
        ...
        <div className="lg:col-span-7 bg-white rounded-2xl border border-slate-100 p-6 shadow-xs flex flex-col h-[480px]">
          <div className="flex items-center justify-between border-b border-slate-50 pb-3">
            <div>
              <h3 className="text-base font-display font-semibold text-slate-800">Queued Withdrawals ({queuedEntries.length})</h3>
              <p className="text-xs text-slate-400">Batch multiple requests together before submitting for board approvals</p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-3 py-4">
            ...
            {queuedEntries.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-center p-6 text-slate-400">
                ...
                <p className="text-xs font-semibold">Your withdrawals queue is empty</p>
      ...
      {isAdmin && withdrawals.some((w) => w.status === TransactionStatus.PENDING) && (
        <div className="bg-amber-50/20 border border-amber-100 rounded-2xl p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-base font-display font-semibold text-slate-800">
              Review Pending Withdrawals Requests
            </h3>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {withdrawals
              .filter((w) => w.status === TransactionStatus.PENDING)
              .map((wth) => (
                <div key={wth.id} className="bg-white border border-slate-150 p-4 rounded-xl flex flex-col justify-between">
                  <div>
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <h4 className="text-xs font-bold text-slate-800">{wth.userName}</h4>
                        <p className="text-[10px] text-slate-400 font-mono font-medium">{wth.date}</p>
                      </div>
                      <span className="text-sm font-mono font-bold text-slate-900">
                        {formatBDT(wth.amount)}
                      </span>
                    </div>
                    <div className="space-y-1 text-[11px] text-slate-500 mb-4 border-t border-slate-50 pt-2">
                      <p><strong>Cheque Number:</strong> {wth.chequeNumber || "-"}</p>
                      <p><strong>Reference:</strong> {wth.reference}</p>
                      {wth.notes && <p className="italic">"{wth.notes}"</p>}
                      {wth.documentUrl && (
                        <button
                          onClick={() => onViewDocument(wth.documentUrl!, wth.fileName || "withdrawal_proof.pdf")}
                  ...
                  <div className="flex items-center gap-2 border-t border-slate-50 pt-3 mt-auto">
                    ...
                    >
                      {processingId === wth.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Check className="w-3.5 h-3.5" />}
                      Approve
                    </button>
                    <button
                      onClick={() => openDeclineModal(wth.id)}
                      disabled={processingId === wth.id}
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-xs">
        <div>
          <h3 className="text-base font-display font-semibold text-slate-800">Submitted Withdrawals Ledger</h3>
          <p className="text-xs text-slate-400">All recorded withdrawal requests and review outcomes</p>
        </div>
        ...
        <div className="overflow-x-auto mt-6">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-50 text-xs text-slate-600">
              {withdrawals.map((wth) => (
                <tr key={wth.id} className="hover:bg-slate-50/30 transition-colors">
                  <td className="py-3.5 font-mono text-[10px] font-medium text-slate-500">{wth.date}</td>
                  {isAdmin && (
                    <td className="py-3.5">
                      <p className="font-semibold text-slate-800 leading-none">{wth.userName}</p>
                    </td>
                  )}
                  <td className="py-3.5 text-right font-mono font-bold text-slate-800">{formatBDT(wth.amount)}</td>
                  <td className="py-3.5 font-mono text-[11px] text-slate-500">{wth.chequeNumber || "-"}</td>
                  <td className="py-3.5 font-mono text-[11px] text-slate-500">{wth.reference}</td>
                  <td className="py-3.5">
                    {wth.status === TransactionStatus.APPROVED ? (
                    ...
                    ) : wth.status === TransactionStatus.DECLINED ? (
                  ...
                  <td className="py-3.5">
                    {wth.documentUrl ? (
                      <button
                        onClick={() => onViewDocument(wth.documentUrl!, wth.fileName || "receipt.pdf")}
                  ...
                  <td className="py-3.5 text-slate-500 font-medium">
                    {wth.remarks ? (
                      <span className="italic">"{wth.remarks}"</span>
                    ) : (
                  ...
                  {isAdmin && (
                    <td className="py-3.5 text-center">
                      <button
                        onClick={() => handleDeleteTransaction(wth.id)}
              ...
              {withdrawals.length === 0 && (
                <tr>
                  <td colSpan={isAdmin ? 9 : 7} className="py-12 text-center text-slate-400 font-medium">
                    No submitted withdrawals found.
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-xs text-slate-500 mb-4">
                You must provide a mandatory review explanation for declining this withdrawal request. The comment will be visible to the shareholder immediately.
              </p>
              ...
              <textarea
                ...
                placeholder="e.g. Overdraft exceeding share equity / missing supporting bank slip. Please resubmit."
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-xs text-slate-500 mb-4 leading-relaxed">
                Are you sure you want to permanently delete this withdrawal transaction? This action is irreversible and will immediately recalculate the shareholder's due balances and the pool's remaining fund.
              </p>
